...
BorderLayout::BorderLayout(int spacing) { this->setSpacing(spacing); }

BorderLayout::~BorderLayout() { QLayoutItem *l; while ((l = takeAt(0))) delete l; }

void BorderLayout::add(QLayoutItem *item, Position position) { list.append(new ItemWrapper(item, position)); }

void BorderLayout::addItem(QLayoutItem *item) { this->add(item, West); }

void BorderLayout::addWidget(QWidget *widget, Position position) { this->add(new QWidgetItem(widget), position); }

bool BorderLayout::hasHeightForWidth() const { return false; }

int BorderLayout::count() const { return this->list.size(); }

QLayoutItem *BorderLayout::itemAt(int index) const
{
    ItemWrapper *wrapper = list.value(index);
    if (wrapper) return wrapper->item;
    else return 0;
}
...
QSize BorderLayout::calculateSize(SizeType sizeType) const
{
    ...
    for (int i = 0; i < list.size(); ++i)
    {
        ...
        if (sizeType == MinimumSize) itemSize = wrapper->item->minimumSize();
        else itemSize = wrapper->item->sizeHint();
        if (position == North || position == South || position == Center) totalSize.rheight() += itemSize.height();
        if (position == West || position == East || position == Center) totalSize.rwidth() += itemSize.width();
    }
...
void BorderLayout::setGeometry(const QRect &rect)
{
    ...
    for (i = 0; i < list.size(); ++i)
    {
        ...
        } else if (position == Center) { center = wrapper; }
    }
    ...
    if (center) center->item->setGeometry(QRect(westWidth, northHeight,rect.width() - eastWidth - westWidth,centerHeight));
}
